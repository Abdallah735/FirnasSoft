...
type Server struct {
	...
	// waiters for received chunks per fileID
...
func (s *Server) handleMetadata(addr *net.UDPAddr, payload []byte, clientAckPacketId uint16) {
	...
	if len(fileID) != FileIDLength {
		fmt.Println("Invalid fileID length from", addr.String())
		return
	}
	...
	s.fileStateChan <- FileCommand{Action: "addIfNotExists", Key: fileID, File: f, Meta: FileMeta{Filename: filename, TotalChunks: totalChunks, ChunkSize: chunkSz}, Reply: replyCh}
	added := (<-replyCh).(bool)
	if !added {
		f.Close()
	...
	fmt.Printf("Metadata received from %s: %s (%d chunks, %d bytes each, fileID: %s)\n", addr.String(), filename, totalChunks, chunkSz, fileID)
	s.requestTaskQueue <- func() {
		s.requestManagerForIncoming(addr, fileID, totalChunks, chunkSz)
	}
}
func (s *Server) handleChunk(addr *net.UDPAddr, payload []byte, clientAckPacketId uint16) {
	...
	replyCh := make(chan any)
	s.fileStateChan <- FileCommand{Action: "checkAndSetReceived", Key: fileID, Idx: idx, Reply: replyCh}
	isNew := (<-replyCh).(bool)
	if !isNew {
		fmt.Printf("Duplicate chunk %d ignored for fileID %s from %s or metadata not received\n", idx, fileID, addr.String())
	...
	s.fileStateChan <- FileCommand{Action: "getFile", Key: fileID, Reply: replyChFile}
	f := (<-replyChFile).(*os.File)
	if f == nil {
		fmt.Println("No file handle for fileID", fileID)
	...
	s.fileStateChan <- FileCommand{Action: "getMeta", Key: fileID, Reply: replyChMeta}
	meta := (<-replyChMeta).(FileMeta)
	if meta.Filename == "" {
		fmt.Println("No metadata for fileID", fileID)
	...
	s.waitStateChan <- WaitCommand{Action: "notify", Key: fileID, Idx: idx}
	s.packetGenerator(addr, Ack, []byte(fmt.Sprintf("chunk %d received", idx)), clientAckPacketId, nil)
	fmt.Printf("Chunk %d received for fileID %s from %s (%d/%d)\n", idx, fileID, addr.String(), meta.Received, meta.TotalChunks)
	replyChRec := make(chan any)
	s.fileStateChan <- FileCommand{Action: "getReceived", Key: fileID, Reply: replyChRec}
	received := (<-replyChRec).(int)
	replyChTot := make(chan any)
	s.fileStateChan <- FileCommand{Action: "getTotal", Key: fileID, Reply: replyChTot}
	total := (<-replyChTot).(int)
	if received >= total && total > 0 {
		s.fileStateChan <- FileCommand{Action: "closeAndDelete", Key: fileID}
		fmt.Printf("File saved from %s: fromClient_%s (fileID: %s)\n", addr.String(), meta.Filename, fileID)
		s.packetGenerator(addr, TransferComplete, []byte(fileID), 0, nil)
		s.waitStateChan <- WaitCommand{Action: "clear", Key: fileID}
		s.waitStateChan <- WaitCommand{Action: "clearStatus", Key: fileID}
	}
}
...
func (s *Server) SendFileToClient(client *Client, filepath string, filename string) error {
	...
	s.serveStateChan <- ServeCommand{Action: "addServe", Key: fileID, File: f, Meta: FileMeta{Filename: filename, TotalChunks: totalChunks, ChunkSize: ChunkSize}}
...
func (s *Server) handleRequestChunk(addr *net.UDPAddr, payload []byte, clientAckPacketId uint16) {
	...
	fileID := string(payload[4 : 4+FileIDLength])
	replyCh := make(chan any)
	s.serveStateChan <- ServeCommand{Action: "getServe", Key: fileID, Reply: replyCh}
	res := (<-replyCh).(struct {
		F  *os.File
		M  FileMeta
		Ok bool
	...
	if !res.Ok {
		fmt.Printf("Request for unknown fileID %s\n", fileID)
		s.packetGenerator(addr, PendingChunk, payload, clientAckPacketId, nil) // Reuse payload as is
		return
	}
	s.serveStateChan <- ServeCommand{Action: "setRequested", Key: fileID, Idx: idx}
	...
	if err != nil {
		if err == io.EOF || err == io.ErrUnexpectedEOF {
			...
			if stErr == nil {
				...
				start := offset
				if start >= fileSize {
					s.packetGenerator(addr, PendingChunk, payload, clientAckPacketId, nil)
					return
				}
	...
	s.serveStateChan <- ServeCommand{Action: "setSent", Key: fileID, Idx: idx}
}
func (s *Server) handleRequestStatus(addr *net.UDPAddr, payload []byte, clientAckPacketId uint16) {
	...
	s.serveStateChan <- ServeCommand{Action: "getRequested", Key: fileID, Idx: idx, Reply: replyReq}
	isReq := (<-replyReq).(bool)
	if !isReq {
		s.packetGenerator(addr, NotReceived, payload, clientAckPacketId, nil)
		return
	...
	s.serveStateChan <- ServeCommand{Action: "getSent", Key: fileID, Idx: idx, Reply: replySent}
...
func (s *Server) handleStatusResponse(addr *net.UDPAddr, msgType byte, payload []byte, clientAckPacketId uint16) {
	...
	idx := int(binary.BigEndian.Uint32(payload[0:4]))
	fileID := string(payload[4 : 4+FileIDLength])
	s.waitStateChan <- WaitCommand{Action: "notifyStatus", Key: fileID, Idx: idx, Status: msgType}
}
func (s *Server) handleTransferComplete(addr *net.UDPAddr, payload []byte, clientAckPacketId uint16) {
	...
	s.serveStateChan <- ServeCommand{Action: "closeAndDeleteServe", Key: fileID}
	s.waitStateChan <- WaitCommand{Action: "clear", Key: fileID}
	s.waitStateChan <- WaitCommand{Action: "clearStatus", Key: fileID}
	fmt.Printf("Peer %s reports transfer complete for fileID %s\n", addr.String(), fileID)
}
...
func (s *Server) requestManagerForIncoming(addr *net.UDPAddr, fileID string, totalChunks int, chunkSize int) {
	...
	for idx := 0; idx < totalChunks; idx++ {
		...
		for {
			replyCh := make(chan any)
			s.fileStateChan <- FileCommand{Action: "isReceived", Key: fileID, Idx: idx, Reply: replyCh}
			...
			s.waitStateChan <- WaitCommand{Action: "ensureChan", Key: fileID, Idx: idx, Reply: replyW}
			...
			case <-time.After(timeout):
				...
				s.waitStateChan <- WaitCommand{Action: "ensureStatusChan", Key: fileID, Idx: idx, Reply: replyS}
				...
				if retries >= maxRetries {
					fmt.Printf("Request for chunk %d for fileID %s from %s timed out after %d retries\n", idx, fileID, addr.String(), retries)
					break
				}
			}
			replyCh2 := make(chan any)
			s.fileStateChan <- FileCommand{Action: "isReceived", Key: fileID, Idx: idx, Reply: replyCh2}
			got := (<-replyCh2).(bool)
...
func (s *Server) requestWorkers() {
	for i := 0; i < 5; i++ {
		go func() {
			for task := range s.requestTaskQueue {
				task()
			}
		}()
	}
...
func (s *Server) Start() {
	...
	go s.requestWorkers() // Start fixed number of workers
	for i := 0; i < 1; i++ {
